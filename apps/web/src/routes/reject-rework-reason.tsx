...
import { useAreas, useRejectReworkReasons } from "@/lib/queries.js";
...
import { ChipListCell } from "@/components/reject-rework-reason/chip-list-cell.js";
import { DeleteRejectReworkReasonDialog } from "@/components/reject-rework-reason/delete-reject-rework-reason-dialog.js";
import { RejectReworkReasonFormDialog } from "@/components/reject-rework-reason/reject-rework-reason-form-dialog.js";

import type { DeleteTarget } from "@/components/reject-rework-reason/delete-reject-rework-reason-dialog.js";
...
const ALL_AREAS = "all";
const EMPTY: RejectReworkReasonListItem[] = [];
...
type RowActionsProps = {
  item: RejectReworkReasonListItem;
  onEdit: (item: RejectReworkReasonListItem) => void;
...
const columnHelper = createColumnHelper<RejectReworkReasonListItem>();
...
function RejectReworkReason() {
  ...
  const [formItem, setFormItem] = useState<RejectReworkReasonListItem | null>(null);
  ...
  const { data, isPending, isError } = useRejectReworkReasons({ page, size, q, areaId });
  ...
  const openEdit = useCallback((item: RejectReworkReasonListItem) => {
    setFormItem(item);
    setFormOpen(true);
  }, []);
  ...
  return (
    <div className="flex flex-col gap-4">
            ...
            {isPending ? (
              ...
              <TableRow>
                <TableCell colSpan={columns.length} className="h-24 text-center text-destructive">
                  Failed to load reject & rework reasons. Please try again.
                </TableCell>
              </TableRow>
            ) : items.length === 0 ? (
              <TableRow>
                ...
                >
                  No reject & rework reasons found.
                </TableCell>
              </TableRow>
            ) : (
      ...
      <RejectReworkReasonFormDialog open={formOpen} onOpenChange={setFormOpen} item={formItem} />
      <DeleteRejectReworkReasonDialog
        target={deleteTarget}
